...
function updateDashboard(data) {
    ...
    // อัพเดท Clusters List (รองรับหลาย cluster)
    const clustersList = document.getElementById('clusters-list');
    if (clustersList && data.clusters_list?.length) {
        clustersList.innerHTML = data.clusters_list.map(c => `
            <div class="cluster-card">
                <div class="cluster-icon">⚡</div>
                <div class="cluster-info">
                    <h3>${c.name || 'คลัสเตอร์ ' + c.id}</h3>
                    <p>${c.shards || 1} ชาร์ด • ${c.servers} เซิร์ฟเวอร์ • ${c.users?.toLocaleString()} ผู้ใช้งาน</p>
                </div>
                <div class="cluster-memory">
                    <span class="memory-label">Latency</span>
                    <span class="memory-value">${c.latency} ms</span>
                </div>
            </div>
        `).join('');
    }
    
    // อัพเดท Players List (รองรับหลาย player)
    const playersList = document.getElementById('players-list');
    if (playersList && data.players?.length) {
        playersList.innerHTML = data.players.map(p => `
            <div class="player-card">
                <div class="player-header">
                    <span class="player-dot ${p.status === 'online' ? 'online' : ''}"></span>
                    <span class="player-name">${p.name}</span>
                </div>
                <div class="player-stats">
                    <div class="player-stat"><span class="label">เชื่อมต่อ:</span><span class="value">${p.connections} ห้อง</span></div>
                    <div class="player-stat"><span class="label">กำลังเล่น:</span><span class="value">${p.playing} ห้อง</span></div>
                    <div class="player-stat"><span class="label">ทำงาน:</span><span class="value">${p.uptime}</span></div>
                </div>
            </div>
        `).join('');
    }
